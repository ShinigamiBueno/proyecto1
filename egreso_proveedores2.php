<?php
    ...
    <center>
      <h1>Realizar egreso a un proveedor</h1>
    </center>
  <div class="row">
    <center>
        <table>
            <form class="col-xs-4" method="post" action="registrar_egreso.php">
              <?php
           ...
           <table>
              ...
              <div>
                <label>Seleccionar Proveedores</label>
                  ...
                  <?php
                    $consultar=$mysqli->query('SELECT idproveedores,nombre_constructora_proveedores FROM proveedores');
                    while($dato=mysqli_fetch_array($consultar))
                    {
                     
                        echo '<option value='.$dato['idproveedores'].'>'.$dato['nombre_constructora_proveedores'].'</option>';
            ...
            <div class="form-group">  
              <label>Egreso Monto</label>
              <input type="text" class="form-control" name="monto"  placeholder="Egreso Monto" required data-validation-required-message="Please enter a message.">
            </div>
            <div class="form-group">
